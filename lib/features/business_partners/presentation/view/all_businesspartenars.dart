...
import 'package:revival/features/business_partners/presentation/view/business_partner_view.dart'; // For navigation in card button
import 'package:revival/features/business_partners/presentation/view/business_partner_view.dart'; // For FAB navigation
...
class BusinessPartner {
  ...
  // Helper to determine type based on code prefix
  String get partnerType {
    ...
    return 'Other'; // Or handle as needed
  }
}
...
class AllBusinessPartnerWowListPage extends StatefulWidget {
  ...
  @override
  State<AllBusinessPartnerWowListPage> createState() => _AllBusinessPartnerWowListPageState();
}

class _AllBusinessPartnerWowListPageState extends State<AllBusinessPartnerWowListPage> {
  // --- Colors (Unchanged) ---
  ...
  final List<BusinessPartner> _allBusinessPartners = [
    BusinessPartner(code: 'C20000', name: 'Global Solutions Inc.', balance: 1250.75, location: '123 Global Ave, Downtown', contactPerson: 'Alice Smith', phone: '555-1111'),
    BusinessPartner(code: 'C30500', name: 'Acme Corporation', balance: -890.20, location: '45 Industrial Way, West Park', contactPerson: 'Bob Jones', phone: '555-2222'),
    BusinessPartner(code: 'V10000', name: 'Tech Innovators Ltd.', balance: 345.99, location: '789 Tech Rd, Silicon Alley', contactPerson: 'Charlie Kim'),
    BusinessPartner(code: 'C20010', name: 'Sunrise Ventures', balance: 2100.50, location: '1 Sunrise Blvd, Eastside', phone: '555-4444'),
    BusinessPartner(code: 'V10050', name: 'Pinnacle Group', balance: 567.80, location: '2 Pinnacle Plz, North Hub', contactPerson: 'Diana Ross', phone: '555-5555'),
    BusinessPartner(code: 'C30000', name: 'Alpha Industries', balance: 0.00, location: '3 Alpha Cres, Airport Zone'),
  ];

  // --- State Variables ---
  List<BusinessPartner> _filteredBusinessPartners = [];
  final TextEditingController _searchController = TextEditingController();
  final NumberFormat _currencyFormatter = NumberFormat.currency(locale: 'en_US', symbol: 'EGP'); // Adjust as needed
  String? _expandedPartnerCode;
  // --- State for Partner Type Filter ---
  String _selectedPartnerType = "All"; // Initial value
  final List<String> _partnerTypeOptions = ["All", "Customers", "Vendors"];

  @override
  void initState() {
    super.initState();
    // Initial filter application (shows all by default)
    _filterPartners();
  ...
  void _filterPartners() {
    ...
    setState(() {
      ...
      if (typeFilter == "Customers") {
        tempFiltered = tempFiltered.where((p) => p.partnerType == 'Customer').toList();
      } else if (typeFilter == "Vendors") {
        tempFiltered = tempFiltered.where((p) => p.partnerType == 'Vendor').toList();
      }
      // No action needed for "All"

      // 2. Filter by Search Query
      if (query.isNotEmpty) {
        tempFiltered = tempFiltered.where((partner) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(_selectedPartnerType) // Show current filter in title
            .animate()
        ...
        elevation: 2.0,
        // --- Updated AppBar Bottom ---
        bottom: PreferredSize(
          preferredSize: const Size.fromHeight(120.0), // Increased height for filters
          child: Container(
            ...
            child: Column(
              children: [
                ...
                // --- Partner Type Dropdown Filter ---
                SizedBox(
                  height: 45,
                  child: DropdownButtonFormField<String>(
                    value: _selectedPartnerType,
                    isExpanded: true,
                    items: _partnerTypeOptions.map((String type) {
                    ...
                    onChanged: (String? newValue) {
                      if (newValue == null || newValue == _selectedPartnerType) return;
                      setState(() {
                        _selectedPartnerType = newValue;
                        _filterPartners(); // Re-apply filters when type changes
                      });
                    },
                    decoration: InputDecoration(
                      filled: true,
                      fillColor: cardBackgroundColor, // White background
                    ...
                    style: TextStyle(color: textColor, fontSize: 14), // Dark text inside dropdown
                    icon: Icon(Icons.filter_alt_outlined, color: primaryColor, size: 20), // Use filter icon
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () {
          Navigator.push(
            context,
            MaterialPageRoute(
                builder: (context) => NewBusinessPartnerPage() // Navigate to your Add New BP Page
            ),
          );
        },
        backgroundColor: accentColor,
        foregroundColor: Colors.white,
        tooltip: 'Add New Partner', // Add tooltip
  ...
  Widget _buildPartnerList() {
    if (_filteredBusinessPartners.isEmpty) {
      return Center(
        child: Padding( // Added padding to center content better
          ...
          child: Column(
            ...
            children: [
              ...
              // More specific message if filters are active
              if (_selectedPartnerType != "All" || _searchController.text.isNotEmpty)
                Padding(
...
// --- Custom Expandable Card Widget (Unchanged) ---
              ...
              Row(
                ...
                children: [
                  CircleAvatar(
                    ...
                    child: Icon(Icons.business_center_outlined, size: 20, color: primaryColor),
                  ),
                  ...
                  Column(
                    ...
                    children: [
                      ...
                      Text(
                        'Balance',
                        style: textTheme.bodySmall?.copyWith(color: labelColor.withOpacity(0.8)),
                      ),
                      const SizedBox(height: 4),
                      Icon(
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        _buildDetailRow(Icons.location_on_outlined, "Location", partner.location, textTheme),
                        if (partner.contactPerson != null)
                          _buildDetailRow(Icons.person_outline, "Contact", partner.contactPerson!, textTheme),
                        if (partner.phone != null)
                          _buildDetailRow(Icons.phone_outlined, "Phone", partner.phone!, textTheme),
                        const SizedBox(height: 16),
                        Center(
                          child: ElevatedButton.icon(
                            ...
                            onPressed: () {
                              // Example Navigation to a Detail/Invoice page for THIS partner
                              // Navigator.push(
                              //   context,
                              //   MaterialPageRoute(
                              //     builder: (context) => BusinessPartnerView(partner: partner),
                              //   ),
                              // );
                              ScaffoldMessenger.of(context).showSnackBar(
                                SnackBar(content: Text('Create Invoice for ${partner.name} (${partner.code})')),
                              );
                            },
                          ).animate().fadeIn(delay: 150.ms).scaleXY(begin: 0.8, curve: Curves.elasticOut),
                        ),
                      ],
                    ),
                  ),
